...
// It is recommended that you don't modify imgui_lib.cpp! It will become difficult for you to update the library.
// Note that 'ImGui::' being a namespace, you can add functions into the namespace from your own source files, without
// modifying imgui_lib.h or imgui_lib.cpp. You may include imgui_internal.h to access internal data structures, but it doesn't
...
- FREQUENTLY ASKED QUESTIONS (FAQ), TIPS
  ...
  - How can I tell whether to dispatch mouse/keyboard to imgui_lib or to my application?
...
/*
 ...
 - General Gamepad controls: enable with ImGuiConfigFlags_NavEnableGamepad. See suggested mappings in imgui_lib.h ImGuiNavInput_ + download PNG/PSD at http://goo.gl/9LgVZW
 ...
 - Try to keep your copy of dear imgui_lib reasonably up to date.
 ...
 EXHIBIT 1: USING THE EXAMPLE BINDINGS (imgui_impl_XXX.cpp files from the examples/ folder).
     ...
     // Application init: create a dear imgui_lib context, setup some options, load fonts
     ImGui::CreateContext();
     ...
     while (true)
     {
         // Feed inputs to dear imgui_lib, start new frame
         ...
         // Render dear imgui_lib into screen
         ImGui::Render();
 ...
 EXHIBIT 2: IMPLEMENTING CUSTOM BINDING / CUSTOM ENGINE.
     ...
     while (true)
     {
        ...
        // Render dear imgui_lib, swap buffers
       ...
       for (int n = 0; n < draw_data->CmdListsCount; n++)
       {
          ...
          for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             else
             {
                 ...
                 // - Clipping coordinates are provided in imgui_lib coordinates space (from draw_data->DisplayPos to draw_data->DisplayPos + draw_data->DisplaySize)
 ...
 - Gamepad support is particularly useful to use dear imgui_lib on a console system (e.g. PS4, Switch, XB1) without a mouse!
 ...
 - Gamepad:
    ...
    - See 'enum ImGuiNavInput_' in imgui_lib.h for a description of inputs. For each entry of io.NavInputs[], set the following values:
         0.0f= not held. 1.0f= fully held. Pass intermediate 0.0f..1.0f values for analog triggers/sticks.
    - We uses a simple >0.0f test for activation testing, and won't attempt to test for a dead-zone.
      Your code will probably need to transform your raw inputs (such as e.g. remapping your 0.2..0.9 raw input range to 0.0..1.0 imgui_lib range, etc.).
    - You can download PNG/PSD files depicting the gamepad controls for common controllers at: http://goo.gl/9LgVZW.
    - If you need to share inputs between your game and the imgui_lib parts, the easiest approach is to go all-or-nothing, with a buttons combo
 ...
 - Mouse:
    ...
    - On a TV/console system where readability may be lower or mouse inputs may be awkward, you may want to set the ImGuiConfigFlags_NavEnableSetMousePos flag.
      Enabling ImGuiConfigFlags_NavEnableSetMousePos + ImGuiBackendFlags_HasSetMousePos instructs dear imgui_lib to move your mouse cursor along with navigation movements.
      When enabled, the NewFrame() function may alter 'io.MousePos' and set 'io.WantSetMousePos' to notify you that it wants the mouse cursor to be moved.
      When that happens your back-end NEEDS to move the OS or underlying mouse cursor on the next frame. Some of the binding in examples/ do that.
      (If you set the NavEnableSetMousePos flag but don't honor 'io.WantSetMousePos' properly, imgui_lib will misbehave as it will see your mouse as moving back and forth!)
 ...
 When you are not sure about a old symbol or function name, try using the Search/Find function of your IDE to look for comments or references in all imgui_lib files.
 ...
 - 2018/09/06 (1.65) - renamed stb_truetype.h to imstb_truetype.h, stb_textedit.h to imstb_textedit.h, and stb_rect_pack.h to imstb_rectpack.h.
                       If you were conveniently using the imgui_lib copy of those STB headers in your project you will have to update your include paths.
 - 2018/09/05 (1.65) - renamed io.OptCursorBlink/io.ConfigCursorBlink to io.ConfigInputTextCursorBlink. (#1427)
 - 2018/08/31 (1.64) - added imgui_widgets.cpp file, extracted and moved widgets code out of imgui_lib.cpp into imgui_widgets.cpp. Re-ordered some of the code remaining in imgui_lib.cpp.
                       NONE OF THE FUNCTIONS HAVE CHANGED. THE CODE IS SEMANTICALLY 100% IDENTICAL, BUT _EVERY_ FUNCTION HAS BEEN MOVED.
                       Because of this, any local modifications to imgui_lib.cpp will likely conflict when you update. Read docs/CHANGELOG.txt for suggestions.
 ...
 - 2018/01/31 (1.60) - moved sample TTF files from extra_fonts/ to misc/fonts/. If you loaded files directly from the imgui_lib repo you may need to update your paths.
 ...
 - 2015/08/05 (1.44) - split imgui_lib.cpp into extra files: imgui_demo.cpp imgui_draw.cpp imgui_internal.h that you need to add to your project.
 ...
 A: This library is poorly documented at the moment and expects of the user to be acquainted with C/C++.
    ...
    - See documentation and comments at the top of imgui_lib.cpp + effectively imgui_lib.h.
 ...
 A: See "Quotes" (https://github.com/ocornut/imgui/wiki/Quotes) and
    ...
    for a list of games/software which are publicly known to use dear imgui_lib. Please add yours if you can!
 ...
 A: You can read the 'io.WantCaptureMouse', 'io.WantCaptureKeyboard' and 'io.WantTextInput' flags from the ImGuiIO structure (e.g. if (ImGui::GetIO().WantCaptureMouse) { ... } )
    - When 'io.WantCaptureMouse' is set, imgui_lib wants to use your mouse state, and you may want to discard/hide the inputs from the rest of your application.
    - When 'io.WantCaptureKeyboard' is set, imgui_lib wants to use your keyboard state, and you may want to discard/hide the inputs from the rest of your application.
    - When 'io.WantTextInput' is set to may want to notify your OS to popup an on-screen keyboard, if available (e.g. on a mobile phone, or console OS).
    Note: you should always pass your mouse/keyboard inputs to imgui_lib, even when the io.WantCaptureXXX flag are set false.
     This is because imgui_lib needs to detect that you clicked in the void to unfocus its own windows.
    Note: The 'io.WantCaptureMouse' is more accurate that any attempt to "check if the mouse is hovering a window" (don't do that!).
     It handle mouse dragging correctly (both dragging that started over your application or over an imgui_lib window) and handle e.g. modal windows blocking inputs.
 ...
 A: Short explanation:
    ...
    Here's a simplified OpenGL example using stb_image_lib.h:

        // Use stb_image_lib.h to load a PNG from disk and turn it into raw RGBA pixel data:
        ...
        // Now that we have an OpenGL texture, assuming our imgui_lib rendering function (imgui_impl_xxx.cpp file) takes GLuint as ImTextureID, we can display it:
 ...
 A: Use the font atlas to load the TTF/OTF file you want:
    ...
    Default is ProggyClean.ttf, monospace, rendered at size 13, embedded in dear imgui_lib's source code.
 ...
 A: - Being highly portable (bindings for several languages, frameworks, programming style, obscure or older platforms/compilers),
      and aiming for compatibility & performance suitable for every modern real-time game engines, dear imgui_lib does not use
 ...
 A: - You can create a dummy window. Call Begin() with the NoBackground | NoDecoration | NoSavedSettings | NoInputs flags.
    ...
    - You can call ImGui::GetBackgroundDrawList() or ImGui::GetForegroundDrawList() and use those draw list to display
      contents behind or over every other imgui_lib windows (one bg/fg drawlist per viewport).
 ...
 A: - If you are experienced with Dear ImGui and C++, look at the github issues, look at the Wiki, read docs/TODO.txt
    ...
    - Businesses: convince your company to fund development via support contracts/sponsoring! This is among the most useful thing you can do for dear imgui_lib.
    - Individuals: you can also become a Patron (http://www.patreon.com/imgui) or donate on PayPal! See README.
    - Disclose your usage of dear imgui_lib via a dev blog post, a tweet, a screenshot, a mention somewhere etc.
      You may post screenshot or links in the gallery threads (github.com/ocornut/imgui_lib/issues/1902). Visuals are ideal as they inspire other programmers.
      But even without visuals, disclosing your use of dear imgui_lib help the library grow credibility, and help other teams and programmers with taking decisions.
...
static void
SettingsHandlerWindow_WriteAll(ImGuiContext *imgui_ctx, ImGuiSettingsHandler *handler, ImGuiTextBuffer *buf);
...
// Misc
    ...
    static bool
    UpdateManualResize(ImGuiWindow *window, const ImVec2 &size_auto_fit, int *border_held, int resize_grip_count,
    ...
    static void RenderWindowDecorations(ImGuiWindow *window, const ImRect &title_bar_rect, bool title_bar_is_highlight,
                                        int resize_grip_count, const ImU32 resize_grip_col[4],
...
ImGuiStyle::ImGuiStyle() {
    ...
    DisplayWindowPadding = ImVec2(19,
                                  19);    // Window position are clamped to be visible within the display area by at least this amount. Only applies to regular windows.
...
void ImGui::RenderTextEllipsis(ImDrawList *draw_list, const ImVec2 &pos_min, const ImVec2 &pos_max, float clip_max_x,
    ...
    if (text_size.x > pos_max.x - pos_min.x) {
        ...
        float text_size_clipped_x = font->CalcTextSizeA(font_size, text_width, 0.0f, text, text_end_full,
                                                        &text_end_ellipsis).x;
        ...
        if (ellipsis_x + ellipsis_width - 1.0f <= ellipsis_max_x)
            RenderPixelEllipsis(draw_list, ImVec2(ellipsis_x, pos_min.y), GetColorU32(ImGuiCol_Text),
                                ellipsis_dot_count);
    } else {
...
bool ImGui::FocusableItemRegister(ImGuiWindow *window, ImGuiID id) {
    ...
    if (g.ActiveId == id && g.FocusTabPressed && !(g.ActiveIdBlockNavInputFlags & (1 << ImGuiNavInput_KeyTab_)) &&
        ...
        g.FocusRequestNextCounterTab = window->DC.FocusCounterTab + (g.IO.KeyShift ? (is_tab_stop ? -1 : 0)
...
// Verify that the type sizes are matching between the calling file's compilation unit and imgui_lib.cpp's compilation unit
// If the user has inconsistent compilation settings, imgui_lib configuration #define, packing pragma, etc. your user code
// may see different structures than what imgui_lib.cpp sees, which is problematic.
...
// The reason this is exposed in imgui_internal.h is: on touch-based system that don't have hovering, we want to dispatch inputs to the right target (imgui_lib vs imgui_lib+app)
void ImGui::UpdateHoveredWindowAndCaptureFlags() {
    ...
    // Update io.WantCaptureMouse for the user application (true = dispatch mouse info to imgui_lib, false = dispatch mouse info to Dear ImGui + app)
    ...
    // Update io.WantCaptureKeyboard for the user application (true = dispatch keyboard info to imgui_lib, false = dispatch keyboard info to Dear ImGui + app)
...
// Note that imgui_lib doesn't know the semantic of each entry of io.KeysDown[]. Use your own indices/enums according to how your back-end/engine stored them into io.KeysDown[]!
...
static bool
ImGui::BeginChildEx(const char *name, ImGuiID id, const ImVec2 &size_arg, bool border, ImGuiWindowFlags flags) {
    ImGuiContext &g = *GImGui;
...
static void CheckStacksSize(ImGuiWindow *window, bool write) {
    ...
    }    // Too few or too many PopID()/TreePop()
    {
        ...
        if (write) *p_backup = (short) current; else IM_ASSERT(*p_backup == current && "BeginGroup/EndGroup Mismatch!");
        p_backup++;
    ...
    }    // Too few or too many PopStyleVar()
    {
        ...
        if (write) *p_backup = (short) current; else IM_ASSERT(*p_backup >= current && "PushFont/PopFont Mismatch!");
        p_backup++;
...
static ImVec2 CalcWindowSizeAfterConstraint(ImGuiWindow *window, ImVec2 new_size) {
    ...
    if (!(window->Flags & (ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_AlwaysAutoResize))) {
        ...
        new_size.y = ImMax(new_size.y, window->TitleBarHeight() + window->MenuBarHeight() + ImMax(0.0f,
...
static ImVec2 CalcWindowContentSize(ImGuiWindow *window) {
    ...
    ImVec2 sz;
    sz.x = (float) (int) ((window->ContentSizeExplicit.x != 0.0f) ? window->ContentSizeExplicit.x :
                          window->DC.CursorMaxPos.x - window->DC.CursorStartPos.x);
    sz.y = (float) (int) ((window->ContentSizeExplicit.y != 0.0f) ? window->ContentSizeExplicit.y :
...
static bool
ImGui::UpdateManualResize(ImGuiWindow *window, const ImVec2 &size_auto_fit, int *border_held, int resize_grip_count,
                          ImU32 resize_grip_col[4]) {
    ImGuiContext &g = *GImGui;
    ...
    const float grip_draw_size = (float) (int) ImMax(g.FontSize * 1.35f,
    ...
    for (int border_n = 0; border_n < resize_border_count; border_n++) {
        ...
        ImRect border_rect = GetResizeBorderRect(window, border_n, grip_hover_inner_size,
                                                 WINDOWS_RESIZE_FROM_EDGES_HALF_THICKNESS);
        ButtonBehavior(border_rect, window->GetID((void *) (intptr_t) (border_n + 4)), &hovered, &held,
...
static inline void ClampWindowRect(ImGuiWindow *window, const ImRect &rect, const ImVec2 &padding) {
    ImGuiContext &g = *GImGui;
    ImVec2 size_for_clamping = (g.IO.ConfigWindowsMoveFromTitleBarOnly &&
                                !(window->Flags & ImGuiWindowFlags_NoTitleBar)) ? ImVec2(window->Size.x,
                                                                                         window->TitleBarHeight())
                                                                                : window->Size;
    window->Pos = ImMin(rect.Max - padding,
...
bool ImGui::Begin(const char *name, bool *p_open, ImGuiWindowFlags flags) {
    ...
    if (window_just_created) {
        ImVec2 size_on_first_use = (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSize)
                                   ? g.NextWindowData.SizeVal : ImVec2(0.0f,
    ...
    if (first_begin_of_the_frame) {
        ...
        if (!window_pos_set_by_api && !(flags & ImGuiWindowFlags_ChildWindow) && window->AutoFitFramesX <= 0 &&
            ...
            if (g.IO.DisplaySize.x > 0.0f && g.IO.DisplaySize.y >
                                             0.0f) // Ignore zero-sized display explicitly to avoid losing positions if a window manager reports zero-sized window when initializing or minimizing.
            {
        ...
        const float resize_grip_draw_size = (float) (int) ImMax(g.FontSize * 1.35f,
                                                                window->WindowRounding + 1.0f + g.FontSize * 0.2f);
        if (!window->Collapsed)
        ...
        window->ContentsRegionRect.Max.x = window->ContentsRegionRect.Min.x +
        ...
        window->ContentsRegionRect.Max.y = window->ContentsRegionRect.Min.y +
        ...
        */
        ...
        window->DC.LastItemStatusFlags = IsMouseHoveringRect(title_bar_rect.Min, title_bar_rect.Max, false)
...
bool ImGui::Begin(const char *name, bool *p_open, const ImVec2 &size_first_use, float bg_alpha_override,
...
void ImGui::FocusWindow(ImGuiWindow *window) {
    ...
    if (window->Flags &
        ImGuiWindowFlags_Popup) // FIXME: This statement should be unnecessary. Need further testing before removing it..
...
void ImGui::PushMultiItemsWidths(int components, float w_full) {
    ...
    ImGuiWindow *window = g.CurrentWindow;
    const ImGuiStyle &style = g.Style;
    const float w_item_one = ImMax(1.0f, (float) (int) ((w_full - (style.ItemInnerSpacing.x) * (components - 1)) /
                                                        (float) components));
    const float w_item_last = ImMax(1.0f, (float) (int) (w_full -
...
void ImGui::SetScrollHereX(float center_x_ratio) {
    ...
    target_x += (last_item_width * center_x_ratio) + (g.Style.ItemSpacing.x * (center_x_ratio - 0.5f) *
...
void ImGui::SetScrollHereY(float center_y_ratio) {
    ...
    target_y += (window->DC.PrevLineSize.y * center_y_ratio) + (g.Style.ItemSpacing.y * (center_y_ratio - 0.5f) *
...
ImVec2 ImGui::FindBestWindowPosForPopup(ImGuiWindow *window) {
    ...
    if (window->Flags & ImGuiWindowFlags_ChildMenu) {
        ...
        if (parent_window->DC.MenuBarAppending)
            r_avoid = ImRect(-FLT_MAX, parent_window->Pos.y + parent_window->TitleBarHeight(), FLT_MAX,
...
void ImGui::NavMoveRequestTryWrapping(ImGuiWindow *window, ImGuiNavMoveFlags move_flags) {
    ImGuiContext &g = *GImGui;
    if (g.NavWindow != window || !NavMoveRequestButNoResultYet() || g.NavMoveRequestForward != ImGuiNavForward_None ||
        g.NavLayer != 0)
        return;
...
float ImGui::GetNavInputAmount(ImGuiNavInput n, ImGuiInputReadMode mode) {
    ...
    if (mode == ImGuiInputReadMode_Repeat)
        return (float) CalcTypematicPressedRepeatAmount(t, t - g.IO.DeltaTime, g.IO.KeyRepeatDelay * 0.80f,
                                                        g.IO.KeyRepeatRate * 0.80f);
    if (mode == ImGuiInputReadMode_RepeatSlow)
        return (float) CalcTypematicPressedRepeatAmount(t, t - g.IO.DeltaTime, g.IO.KeyRepeatDelay * 1.00f,
...
#undef NAV_MAP_KEY
    ...
    if (g.NavMoveRequestForward == ImGuiNavForward_None) {
        ...
        if (g.NavWindow && !g.NavWindowingTarget && allowed_dir_flags &&
            ...
            if ((allowed_dir_flags & (1 << ImGuiDir_Left)) &&
                IsNavInputPressedAnyOfTwo(ImGuiNavInput_DpadLeft, ImGuiNavInput_KeyLeft_, ImGuiInputReadMode_Repeat))
                g.NavMoveDir = ImGuiDir_Left;
            if ((allowed_dir_flags & (1 << ImGuiDir_Right)) &&
                IsNavInputPressedAnyOfTwo(ImGuiNavInput_DpadRight, ImGuiNavInput_KeyRight_, ImGuiInputReadMode_Repeat))
                g.NavMoveDir = ImGuiDir_Right;
            if ((allowed_dir_flags & (1 << ImGuiDir_Up)) &&
                IsNavInputPressedAnyOfTwo(ImGuiNavInput_DpadUp, ImGuiNavInput_KeyUp_, ImGuiInputReadMode_Repeat))
                g.NavMoveDir = ImGuiDir_Up;
            if ((allowed_dir_flags & (1 << ImGuiDir_Down)) &&
                IsNavInputPressedAnyOfTwo(ImGuiNavInput_DpadDown, ImGuiNavInput_KeyDown_, ImGuiInputReadMode_Repeat))
    ...
    if (g.NavWindow && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) && !g.NavWindowingTarget) {
        ...
        const float scroll_speed = ImFloor(window->CalcFontSize() * 100 * g.IO.DeltaTime +
                                           0.5f); // We need round the scrolling speed because sub-pixel scroll isn't reliably supported.
    ...
    g.NavScoringRectScreen = g.NavWindow ? ImRect(g.NavWindow->Pos + nav_rect_rel.Min,
                                                  g.NavWindow->Pos + nav_rect_rel.Max) : GetViewportRect();
    g.NavScoringRectScreen.TranslateY(nav_scoring_rect_offset_y);
    g.NavScoringRectScreen.Min.x = ImMin(g.NavScoringRectScreen.Min.x + 1.0f, g.NavScoringRectScreen.Max.x);
    g.NavScoringRectScreen.Max.x = g.NavScoringRectScreen.Min.x;
    IM_ASSERT(
            !g.NavScoringRectScreen.IsInverted()); // Ensure if we have a finite, non-inverted bounding box here will allows us to remove extraneous ImFabs() calls in NavScoreItem().
...
static void ImGui::NavUpdateWindowing() {
    ...
    // FIXME: We lack an explicit IO variable for "is the imgui_lib window focused", so compare mouse validity to detect the common case of back-end clearing releases all keys on ALT-TAB
    ...
    if (g.NavWindowingTarget && !(g.NavWindowingTarget->Flags & ImGuiWindowFlags_NoMove)) {
        ...
        if (move_delta.x != 0.0f || move_delta.y != 0.0f) {
            ...
            SetWindowPos(g.NavWindowingTarget->RootWindow,
                         g.NavWindowingTarget->RootWindow->Pos + move_delta * move_speed, ImGuiCond_Always);
            g.NavDisableMouseHover = true;
...
static void SettingsHandlerWindow_WriteAll(ImGuiContext *ctx, ImGuiSettingsHandler *handler, ImGuiTextBuffer *buf) {
    ...
    for (int i = 0; i != g.Windows.Size; i++) {
        ...
        ImGuiWindowSettings *settings = (window->SettingsIdx != -1) ? &g.SettingsWindows[window->SettingsIdx]
    ...
    struct Funcs {
        ...
        static void NodeDrawList(ImGuiWindow *window, ImDrawList *draw_list, const char *label) {
            ...
            for (const ImDrawCmd *pcmd = draw_list->CmdBuffer.begin();
                ...
                ImGui::Text("ElemCount: %d, ElemCount/3: %d, VtxOffset: +%d, IdxOffset: +%d", pcmd->ElemCount,
                            pcmd->ElemCount / 3, pcmd->VtxOffset, pcmd->IdxOffset);
        ...
        static void NodeWindow(ImGuiWindow *window, const char *label) {
            ...
            ImGui::BulletText("Scroll: (%.2f/%.2f,%.2f/%.2f)", window->Scroll.x, window->ScrollMax.x, window->Scroll.y,
                              window->ScrollMax.y);
        ...
        static void NodeTabBar(ImGuiTabBar *tab_bar) {
            ...
            ImFormatString(p, buf_end - p, "TabBar (%d tabs)%s", tab_bar->Tabs.Size,
...
// Include imgui_user.inl at the end of imgui_lib.cpp to access private data/functions that aren't exposed.
